...
class _OfflinePlayerState extends State<OfflinePlayer> {
  ...
  Widget playMedia(){
    ...
    _videoFile == null ? _controller = VideoPlayerController.network('https://youtu.be/D9x7JlFMZjU')
      ...
      setState(() => _initializeVideoPlayerFuture = _controller.initialize());
    ...
    return Column(
      children: <Widget>[
        Container(
          ...
          child:(_videoFile != null) ? _picker.getVideo(source: ImageSource.gallery) : Image.network('https://i.imgur.com/sUFH1Aq.png'),
        ),
        ElevatedButton(
  ...
  Future getVidFromGallery() async {
    _pickedVideo = await _picker.getVideo(source: ImageSource.gallery, maxDuration: Duration(seconds: 120));
    setState(() {
      if (_pickedVideo != null){
        _videoFile = File(_pickedVideo.path);
      }
    });
  }
}
